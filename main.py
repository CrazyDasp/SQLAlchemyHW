...
def find_all_sales(info_publisher):
    publisher = session.query(Publisher).filter(Publisher.name == info_publisher).first()

    if publisher:
        book_alias = aliased(Book)
        sales = (session.query(Sale)
                 .join(Stock).join(book_alias, Stock.book)
                 .filter(book_alias.publisher == publisher)
                 .all())
...
info_publisher = input()

find_all_sales(info_publisher)
